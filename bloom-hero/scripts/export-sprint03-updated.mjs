/**
 * tests/sprint-3.csv -> tests/Sprint-03-updated.csv + src/data/sprint03-updated.json
 * Run: npm run sprint03:export
 */
import fs from "fs";
import path from "path";
import { fileURLToPath } from "url";
import { REPLACED_MODULE_CASES, MODULE_RATIO_TARGETS } from "./sprint3-module-replacements.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const root = path.join(__dirname, "..");
const outCsv = path.join(root, "tests", "Sprint-03-updated.csv");
const outJson = path.join(root, "src", "data", "sprint03-updated.json");
...
function modernizeText(s) {
  ...
  t = t.replace(/\/customer\/dashboard/gi, "/");
...
function deriveHonestStatus(c) {
  ...
  if (/not implemented|no pagination|no review section|no multi-stage|not yet reviewed|hard coded/i.test(blob)) {
    return "Failed";
  }
  if (/partial|verify manually|may vary/i.test(blob)) return "Partial";
  if (/implemented|renders|redirect|fetch|shows|queries|opens modal/i.test(c.actual.toLowerCase())) {
    return "Passed";
  }
  return "Not Run";
}
...
function assignIds(cases) {
  return cases.map((row, i) => ({
    ...finalizeCase({ ...row }),
    id: `TC-SP3-${String(i + 1).padStart(3, "0")}`,
  }));
}
...
try {
  ...
  const fallback = path.join(root, "tests", "Sprint-03-updated.export.csv");
...
fs.writeFileSync(
  outJson,
  JSON.stringify(
    { generatedAt: new Date().toISOString(), source: "tests/sprint-3.csv (rebuilt)", testAccounts: ACCOUNTS, total: cases.length, sunnyRainyRatio: ratioReport, cases },
    null,
    2
  )
);
