/**
 * tests/sprint-4.csv -> tests/Sprint-04-updated.csv + src/data/sprint04-updated.json
 * Run: npm run sprint04:export
 */
import fs from "fs";
import path from "path";
import { fileURLToPath } from "url";
import { REPLACED_MODULE_CASES, MODULE_RATIO_TARGETS } from "./sprint4-module-replacements.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const root = path.join(__dirname, "..");
const outCsv = path.join(root, "tests", "Sprint-04-updated.csv");
const outJson = path.join(root, "src", "data", "sprint04-updated.json");
...
function deriveHonestStatus(c) {
  ...
  if (/not implemented|no skeleton|still mock/i.test(blob)) return "Failed";
  if (/partial|verify in supabase/i.test(blob)) return "Partial";
  if (/fetch|queries|renders|redirect|shows|loads/i.test(c.actual.toLowerCase())) return "Passed";
  return "Not Run";
}
...
function assignIds(cases) {
  return cases.map((row, i) => ({
    ...finalizeCase({ ...row }),
    id: `TC-SP4-${String(i + 1).padStart(3, "0")}`,
  }));
}
...
try {
  ...
  const fallback = path.join(root, "tests", "Sprint-04-updated.export.csv");
...
fs.writeFileSync(
  outJson,
  JSON.stringify(
    { generatedAt: new Date().toISOString(), source: "tests/sprint-4.csv (rebuilt)", testAccounts: ACCOUNTS, total: cases.length, sunnyRainyRatio: ratioReport, cases },
    null,
    2
  )
);
