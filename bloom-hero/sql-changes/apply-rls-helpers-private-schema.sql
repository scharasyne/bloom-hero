-- Manual only. Run in Supabase SQL editor.
--
-- Fixes BOTH:
--   • permission denied for function is_admin (RLS needs EXECUTE on helpers)
--   • linter 0029 (helpers not callable via /rest/v1/rpc/*)
--
-- Helpers live in schema "private" (not exposed to PostgREST). RLS policies call private.*.
-- Then run apply-rls-consolidate-permissive-policies.sql (uses private.*).
-- Revokes EXECUTE on old public.* helpers so RPC + linter warnings go away.
