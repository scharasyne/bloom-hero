// Source: `src/app/(customer)/orders/[orderId]/pay/page.tsx`

import type { CustomerPayPageResult } from "@/features/orders/types";

type CustomerPayOrderPageViewProps = {
  orderId: string;
  paymentPage: Extract<CustomerPayPageResult, { status: "ready" }>;
  submitReceipt: (formData: FormData) => Promise<void>;
};

export function CustomerPayOrderUnauthenticatedView() {
  return (
...
export function CustomerPayOrderNotFoundView() {
  return (
    <main className="mx-auto w-full max-w-xl px-6 py-10">
      <h1 className="text-2xl font-bold text-[#2f2f2f]">Upload Payment Receipt</h1>
...
export function CustomerPayOrderPageView({
  orderId,
  paymentPage,
  submitReceipt,
}: CustomerPayOrderPageViewProps) {
