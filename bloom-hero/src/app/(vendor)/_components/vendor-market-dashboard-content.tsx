...
function SparkLine({ data }: { data: number[] }) {
  ...
  const lastX = W;
  const lastY =
    H - ((data[data.length - 1] - min) / (max - min || 1)) * (H - 10) - 5;
...
export function VendorMarketDashboardContent() {
  ...
  useEffect(() => {
    ...
    const loadDashboard = async () => {
      setLoadError(null);
      const response = await fetch(`/api/vendor-dashboard?type=market`);
        ...
        <div className="rounded-2xl border border-[#ebe7e3] bg-white p-5 shadow-[0_1px_4px_rgba(0,0,0,0.04)] lg:col-span-2">
          <div className="mb-5 flex items-start justify-between">
            ...
            {revenueSummary.changePct !== null ? (
              ...
              >
                <Icon
                  icon={revenueSummary.positive ? "mdi:trending-up" : "mdi:trending-down"}
                  width={13}
                />
        ...
        <div className="rounded-2xl border border-[#ebe7e3] bg-white p-5 shadow-[0_1px_4px_rgba(0,0,0,0.04)]">
          ...
          <div className="space-y-1">
            {upcomingOrders.map((o, i) => (
              <div key={`${o.day}-${o.date}`}>
                <div
                  className={`flex items-center gap-3 rounded-xl px-3 py-2.5 transition-colors ${
                    o.isToday ? "bg-[#f4f9f5]" : "hover:bg-[#faf9f7]"
                  }`}
                >
                  <div
                    className={`flex min-w-[40px] flex-col items-center rounded-lg px-2 py-1.5 text-center ${
                      o.isToday ? "bg-[#2f5d3a] text-white" : "bg-[#f7f5f2] text-[#1e1c1a]"
                    }`}
                  >
                    <span
                      className={`text-[9px] font-semibold uppercase tracking-wider ${
                        o.isToday ? "text-white/70" : "text-slate-400"
                      }`}
                    >
                      {o.day}
                    </span>
                    <span className="mt-0.5 text-sm font-bold leading-none">{o.date}</span>
                  </div>
                ...
                {i < upcomingOrders.length - 1 && <div className="mx-3 h-px bg-[#f5f2ef]" />}
              </div>
            ))}
          </div>
        </div>
              ...
              <div key={order.id}>
                <div className="flex items-center justify-between gap-4 px-5 py-3.5 transition-colors hover:bg-[#faf9f7]">
                  ...
                  <div className="flex shrink-0 items-center gap-3">
                    ...
                    <span className="w-16 text-right text-sm font-bold tabular-nums text-[#1e1c1a]">
                      {order.amount}
                    </span>
                  </div>
                </div>
