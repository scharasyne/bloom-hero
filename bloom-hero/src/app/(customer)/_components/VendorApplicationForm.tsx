...
export default function VendorApplicationForm({
  ...
  async function saveApplicationDraft() {
    ...
    setIsSaving(true);
    try {
      const result = await saveVendorApplicationDraft({
        shopName,
        shopAddress,
        email,
        phoneNumber,
  ...
  async function handleFinalSubmit() {
    ...
    try {
      ...
      const result = await submitVendorApplication({
        ...
        tin,
        vatRegistrationStatus,
        primaryBusinessDocumentUrl,
        governmentIdDocumentUrl,
        birCertificateUrl,
      });
      if (!result.ok) throw new Error(result.error ?? "Failed to submit vendor application.");
          ...
          {STEP_ITEMS.map((item, idx) => {
            ...
            return (
              <li key={item.id} className="relative flex flex-1 flex-col items-center">
                {idx !== 0 && (
                  <div
                    className={`absolute right-1/2 top-4 -z-10 h-0.5 w-full transition-colors duration-500 ${
