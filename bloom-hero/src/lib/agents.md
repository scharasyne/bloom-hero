...
## Scope
- Transfer relevant code from `src/lib` into the matching `src/features/<feature>` folder.
...
- If a file in `src/lib` contains multiple functions with different purposes, split it into separate files so each function moves to the correct feature folder.
...
- Keep generic helpers in `src/lib` only when they are truly shared across multiple features and do not belong to one domain.

## Split Example
- If one lib file exports `getVendorProfile` and `updateVendorProfile`, create:
	- `src/features/vendors/queries/getVendorProfile.ts`
	- `src/features/vendors/actions/updateVendorProfile.ts`
- If one lib file exports several unrelated reads, create one query file per function.
- If one lib file exports several unrelated mutations, create one action file per function.
