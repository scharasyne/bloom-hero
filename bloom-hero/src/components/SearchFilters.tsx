...
const MORE_FILTER_OPTIONS = ["Newest", "Top Rated", "Nearby"];
...
interface Props {
  ...
  moreFilter: string;
  onMoreFilterChange: (v: string) => void;
}
  ...
  return (
    <div className="relative group">
      <button
        type="button"
        className={
          "flex items-center gap-1.5 px-4 py-2 rounded-full text-[13px] font-medium transition-all border " +
          (active
...
function DropdownItem({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={
        "w-full text-left px-4 py-2 text-[13px] transition-colors rounded-xl mx-auto flex items-center justify-between gap-3 hover:bg-[#f5f2ed] " +
...
export default function SearchFilters({
  ...
  return (
    <div className="flex flex-wrap gap-2 items-center justify-center">
      ...
      <FilterPill label={SORT_LABELS[sort]} active={sort !== "Best Sellers"}>
        {SORT_OPTIONS.map((opt) => (
          <DropdownItem key={opt} label={SORT_LABELS[opt]} selected={sort === opt} onClick={() => onSortChange(opt)} />
        ))}
      </FilterPill>

      <FilterPill label={moreFilter === "All" ? "More Filters" : `More Filters: ${moreFilter}`} active={moreFilter !== "All"}>
        <DropdownItem label="All" selected={moreFilter === "All"} onClick={() => onMoreFilterChange("All")} />
        {MORE_FILTER_OPTIONS.map((opt) => (
          <DropdownItem key={opt} label={opt} selected={moreFilter === opt} onClick={() => onMoreFilterChange(opt)} />
        ))}
      </FilterPill>
    </div>
  );
