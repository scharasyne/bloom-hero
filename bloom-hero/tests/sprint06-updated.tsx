...
import sprint06Data from "@/data/sprint06-updated.json";

export type Sprint06Row = {
  id: string;
  userStoryId: string;
  module: string;
  title: string;
...
const COLUMNS: { key: keyof Sprint06Row; label: string; wide?: boolean }[] = [
  { key: "id", label: "ID" },
...
export default function Sprint06Updated() {
  const rows = sprint06Data.cases as Sprint06Row[];
  ...
  return (
    <div className="min-h-screen bg-[#fbf7f4] text-[#1f1f1f]" style={{ fontFamily: "'Quicksand', sans-serif" }}>
      <header className="border-b border-[#e6e2dd] bg-white px-4 py-6 sm:px-8">
        <h1 className="text-2xl font-semibold">Sprint 06 — Updated test cases</h1>
        <p className="mt-1 text-sm text-[#7a746e]">
          Testable features only: admin auth, suspensions, review moderation, search, vendor dashboard, and pop-up requests. Each module has 2 Sunny and 8 Rainy cases.
        </p>
        <div className="mt-4 flex flex-wrap gap-3 text-xs text-[#5f5a55]">
          {Object.entries(sprint06Data.testAccounts as Record<string, string>).map(([k, v]) => (
