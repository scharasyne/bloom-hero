...
import sprint02Data from "@/data/sprint02-updated.json";

export type Sprint02Row = {
  id: string;
  userStoryId: string;
  module: string;
  title: string;
...
const COLUMNS: { key: keyof Sprint02Row; label: string; wide?: boolean }[] = [
  { key: "id", label: "ID" },
...
export default function Sprint02Updated() {
  const rows = sprint02Data.cases as Sprint02Row[];
  ...
  return (
    <div className="min-h-screen bg-[#fbf7f4] text-[#1f1f1f]" style={{ fontFamily: "'Quicksand', sans-serif" }}>
      <header className="border-b border-[#e6e2dd] bg-white px-4 py-6 sm:px-8">
        <h1 className="text-2xl font-semibold">Sprint 02 — Updated test cases</h1>
        <p className="mt-1 text-sm text-[#7a746e]">
          Grounded in current routes (/cart, /orders, /vendor/*, /admin/*). Each module has 2 Sunny and 8 Rainy cases.
        </p>
        <div className="mt-4 flex flex-wrap gap-3 text-xs text-[#5f5a55]">
          {Object.entries(sprint02Data.testAccounts as Record<string, string>).map(([k, v]) => (
